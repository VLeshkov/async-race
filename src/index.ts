...
function getColor(): string {
  ...
  for (let i = 0; i < 6; i += 1) {
    colorCode = colorCode + numbers[Math.floor(Math.random() * numbers.length)];
  }

  return colorCode;
}
  
function generateCar(manufacturers: string[], models: string[]): Car {
  const manufIndex = Math.floor(Math.random() * manufacturers.length);
