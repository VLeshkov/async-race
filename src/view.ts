...
function buildPage(): void {
  ...
  wrapper.innerHTML = `
  <div>
    <button type="button" class="btn btn-outline-primary" id="btn-garage">TO GARAGE</button>
    <button type="button" class="btn btn-outline-primary" id="btn-winners">TO WINNERS</button>
  </div>
  `;
...
function renderGarageLayout(container: HTMLElement): void {
  const addCarForm = document.createElement('div');
  addCarForm.classList.add('car-form');
  addCarForm.innerHTML = `
  <input type="text" id="add-car-name">
  <input type="color" id="add-car-color">
  <button id="btn-add-car" class="btn btn-outline-dark">Add car</button>
  `;

  const updateCarForm = document.createElement('div');
  updateCarForm.classList.add('car-form');
  updateCarForm.innerHTML = `
  <input type="text" id="update-car-name">
  <input type="color" id="update-car-color">
  <button class="btn btn-outline-dark">Update car</button>
  `;

  const raceControls = document.createElement('div');
  raceControls.innerHTML = `
  <button type="button" class="btn btn-outline-primary" id="btn-race">RACE</button>
  <button type="button" class="btn btn-outline-primary" id="btn-reset">RESET</button>
  <button type="button" class="btn btn-outline-primary" id="generate-cars">GENERATE CARS</button>
  `;

  const carsView = document.createElement('div');
  carsView.innerHTML = `
  <h2>Garage</h2>
  <h3 id="page-counter"></h3>
  <div class="cars-list" id="cars-list"></div>
  <div>
    <button class="btn btn-outline-primary" id="btn-prev">Previous</button>
    <button class="btn btn-outline-primary" id="btn-next">Next</button>
  </div>
  `;

  container.appendChild(addCarForm);
  container.appendChild(updateCarForm);
  container.appendChild(raceControls);
  container.appendChild(carsView);
}

function renderWinnersLayout(container: HTMLElement): void {
  const winnersView = document.createElement('div');
  winnersView.innerHTML = `
  <h2 id="winners-header">Winners</h2>
  <h3 id="page-counter">Page #</h3>
  <table class="winners-table" id="winners-table"></table>
  <div>
    <button class="btn btn-outline-primary" id="btn-prev">Previous</button>
    <button class="btn btn-outline-primary" id="btn-next">Next</button>
  </div>
  `;
  container.appendChild(winnersView);
}

function drawWinnersTable(table: HTMLElement, cars: Array<Car>): void {
  table.innerHTML = `
  <tr>
    <th>Number</th>
    <th>Car</th>
  ...
  cars.forEach((car) => {
    ...
    newRow.innerHTML = `
      ...
      <td>
        ...
        width="70" height="30" viewBox="0 0 1280.000000 640.000000"
...
export { buildPage, renderGarageLayout, renderWinnersLayout, drawWinnersTable };